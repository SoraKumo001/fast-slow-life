...
export const InventoryPanel: React.FC = () => {
  const { inventory, targetAmounts, facilities, setTargetAmount, sellItem, currentTier, dungeons } =
    useGameStore();
          ...
          >
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span
                  className={`text-[10px] px-2 py-0.5 rounded font-semibold ${getCategoryBadgeColor(selectedItem.category)}`}
                >
                ...
                <div className="flex justify-between items-center text-xs">
                  ...
                  <div className="flex items-center gap-1 bg-slate-900 border border-slate-800 rounded px-2 py-1">
                    <input
                      type="number"
                      value={
                        targetAmounts[selectedItem.id] === 0 ? "" : targetAmounts[selectedItem.id]
                      }
                      onChange={(e) =>
                        setTargetAmount(selectedItem.id, parseInt(e.target.value) || 0)
                      }
                      placeholder="0"
