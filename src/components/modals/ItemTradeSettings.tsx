...
interface ItemTradeSettingsProps {
  item: Item;
  ...
  onToggleTradeRule: (ruleId: string) => void;
  children?: React.ReactNode;
}

export const ItemTradeSettings: React.FC<ItemTradeSettingsProps> = ({
  item,
  inventory,
  targetAmounts,
  tradeRules,
      ...
      <div className="bg-slate-950/60 p-3 rounded-lg border border-slate-850 space-y-2.5">
        ...
        <div className="flex flex-col gap-2 border-t border-slate-900 pt-2.5">
          <div className="flex justify-between items-center text-xs">
            ...
            <div className="flex items-center gap-1 bg-slate-900 border border-slate-800 rounded px-2 py-1">
              <input
                ...
                onChange={(e) => onSetTargetAmount(item.id, parseInt(e.target.value) || 0)}
                placeholder="0"
          ...
          <div className="flex gap-1.5 justify-end">
            <Button onClick={() => onSetTargetAmount(item.id, 0)} variant="secondary" size="xs">
              リセット
            </Button>
            <Button
              onClick={() => {
                const cur = targetAmounts[item.id] || 0;
                onSetTargetAmount(item.id, cur + 1);
              }}
              variant="custom"
              size="xs"
            ...
            <Button
              onClick={() => {
                const cur = targetAmounts[item.id] || 0;
                onSetTargetAmount(item.id, cur + 10);
              }}
              variant="custom"
              size="xs"
              className="bg-sky-600/20 hover:bg-sky-600/30 border border-sky-500/20 text-sky-400 font-bold"
            ...
            <Button
              onClick={() => {
                const cur = targetAmounts[item.id] || 0;
                onSetTargetAmount(item.id, cur + 100);
              }}
              variant="custom"
              size="xs"
              className="bg-sky-600/20 hover:bg-sky-600/30 border border-sky-500/20 text-sky-400 font-bold"
          ...
          {currentRule ? (
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                <Button
                  onClick={() => onToggleTradeRule(currentRule.id)}
                ...
                <Button
                  onClick={() => onDeleteTradeRule(currentRule.id)}
                  variant="danger"
                  size="sm"
            ...
            <div className="space-y-3">
              ...
              <Button
                onClick={() => {
                  onAddTradeRule(item.id, "sell", autoSellThreshold);
                }}
                variant="custom"
                size="sm"
