...
describe("marketHelpers", () => {
  ...
  describe("getMarketSellBonus", () => {
    it("レベル0の場合は0を返すこと", () => {
      expect(getMarketSellBonus(0)).toBe(0);
    });
    ...
    it("レベル3の場合は0.3、レベル5の場合は0.5を返すこと", () => {
      expect(getMarketSellBonus(3)).toBe(0.3);
      expect(getMarketSellBonus(5)).toBe(0.5);
    });
  });

  describe("getSellBonus", () => {
    it("交易所レベル0の場合は0を返すこと", () => {
      expect(getSellBonus("consumable", { market: { level: 0 } })).toBe(0);
    });
    ...
    it("marketが未定義の場合はレベル0相当の0を返すこと", () => {
      expect(getSellBonus("consumable", { market: undefined as never })).toBe(0);
    });
  });
});
